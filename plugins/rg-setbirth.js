...
if (user.birth) {
return conn.reply(m.chat, `*☁️ Ya tienes una fecha establecida, si quieres borrar la fecha actual usa:*\n> ${usedPrefix}delbirth`, m);
}

if (!text) return conn.reply(m.chat, `*${xreg} Ingresa una fecha válida para tu cumpleaños.*\n> *\`Ejemplo:\`* *${usedPrefix + command} 08/04/2003* (Dia/Mes/Año)`, m);
...
if (!birth) {
return conn.reply(m.chat, `${xreg} Recuerda elegir una fecha de nacimiento válida.*\n> > *\`Ejemplo:\`* ${usedPrefix + command} 01/12/2004`, m);
}
