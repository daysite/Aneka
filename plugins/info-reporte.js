let handler = async (m, { conn, text, usedPrefix, command }) => {
    try {
        text = text.trim();
        if (!text) return m.reply(`*${xinfo} Por favor ingresa el error que desea reportar.*`);
...
> ${text}`;
        ...
        console.error(err);
        m.reply('*✖️ Ocurrió un error al enviar tu reporte. Inténtalo de nuevo más tarde.*');
    }
};
